...
<a href='prog3.php' style='position:absolute;right:20px;top:10px' class='btn btn-primary' >retour</a>
    <div style="border-radius: 20px;width: 50%;margin: 30px 25%;background-color: white;padding: 20px;">
     ...
     <center><label for="login" style="color:#A2CA71;font-weight: bolder;font-size: 25px;">donner le nom du professeur</label></center>
     <br> 
        <input  class="form-control" style="width: 70%; margin-left: 15%;border-radius: 20px;outline: 3px green solid;"  type="text" placeholder="nome" name="nomp">
...
<input type="submit" class="btn btn-success" style="width: 50%; margin-left: 25%;" value="ajouter">
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
      try{
        $nomp=$_POST["nomp"];
        $db->exec("insert into prof values('$nomp');");
        echo "<div class='alert-success'>le professeur est bien enregistré</div>";
